  ...
  useEffect(() => {
    ...
    class MouseClass {
      constructor() {
        ...
        this._clearCachedRect = () => { this.cachedRect = null; };
      }
      init(container) {
        this.container = container;
        ...
        this.listenerTarget.addEventListener('resize', this._clearCachedRect);
        this.listenerTarget.addEventListener('scroll', this._clearCachedRect, { passive: true });
        this.container.addEventListener('mouseenter', this._clearCachedRect);
      }
      dispose() {
        if (this.listenerTarget) {
          ...
          this.listenerTarget.removeEventListener('resize', this._clearCachedRect);
          this.listenerTarget.removeEventListener('scroll', this._clearCachedRect);
        }
        if (this.container) {
          this.container.removeEventListener('mouseenter', this._clearCachedRect);
        }
        if (this.docTarget) {
    ...
    class AutoDriver {
      ...
      update() {
        ...
        let dir = this._tmpDir.subVectors(this.target, this.current);
        let dist = dir.length();
        if (dist < 0.05) {
          this.pickNewTarget();
          dir.subVectors(this.target, this.current);
          dist = dir.length();
        }
        dir.normalize();
        ...
        let targetIntensity = this.mouse.autoIntensity || 2.0;
        if (this.mouse.isHoverInside) {
          targetIntensity *= 0.3;
        }
        this.diff.multiplyScalar(targetIntensity);
      }
    }
...
`;
    ...
    class Advection extends ShaderPass {
      ...
      update({ dt: dtVal, isBounce: bounceVal, BFECC: bfeccVal } = {}) {
        if (!this.uniforms) return;
        if (typeof dtVal === 'number') this.uniforms.dt.value = dtVal;
        if (typeof bounceVal === 'boolean') this.line.visible = bounceVal;
        if (typeof bfeccVal === 'boolean') this.uniforms.isBFECC.value = bfeccVal;
        super.update();
      }
    }
    ...
    class ExternalForce extends ShaderPass {
      ...
      init(simProps) {
        ...
        // User mouse force material & mesh
        const userM = new THREE.RawShaderMaterial({
        ...
        this.userMouse = new THREE.Mesh(mouseG, userM);
        this.scene.add(this.userMouse);

        // Auto driver force material & mesh
        const autoM = new THREE.RawShaderMaterial({
          vertexShader: mouse_vert,
          fragmentShader: externalForce_frag,
          blending: THREE.AdditiveBlending,
          depthWrite: false,
        ...
        this.autoMouse = new THREE.Mesh(mouseG, autoM);
        this.scene.add(this.autoMouse);
      }
      update(props = {}) {
        const cellScale = props.cellScale || { x: 1, y: 1 };
        const cursorSz = props.cursor_size || 0;
        const cursorSizeX = cursorSz * cellScale.x;
        const cursorSizeY = cursorSz * cellScale.y;

        // 1. Update user mouse uniforms
        let userForceX = 0;
        let userForceY = 0;
        if (Mouse.isHoverInside) {
          userForceX = (Mouse.diff.x / 2) * (props.mouse_force || 0);
          userForceY = (Mouse.diff.y / 2) * (props.mouse_force || 0);
        }
        const userCenterX = Math.min(
          Math.max(Mouse.coords.x, -1 + cursorSizeX + cellScale.x * 2),
          1 - cursorSizeX - cellScale.x * 2
        );
        const userCenterY = Math.min(
          Math.max(Mouse.coords.y, -1 + cursorSizeY + cellScale.y * 2),
          1 - cursorSizeY - cellScale.y * 2
        );
        const userUniforms = this.userMouse.material.uniforms;
        userUniforms.force.value.set(userForceX, userForceY);
        userUniforms.center.value.set(userCenterX, userCenterY);
        userUniforms.scale.value.set(cursorSz, cursorSz);

        // 2. Update auto driver uniforms
        let autoForceX = 0;
        let autoForceY = 0;
        let autoCenterX = 0;
        let autoCenterY = 0;
        const ad = props.autoDriver;
        if (ad && ad.enabled) {
          autoForceX = (ad.diff.x / 2) * (props.mouse_force || 0) * 4.0;
          autoForceY = (ad.diff.y / 2) * (props.mouse_force || 0) * 4.0;
          autoCenterX = Math.min(
            Math.max(ad.current.x, -1 + cursorSizeX + cellScale.x * 2),
            1 - cursorSizeX - cellScale.x * 2
          );
          autoCenterY = Math.min(
            Math.max(ad.current.y, -1 + cursorSizeY + cellScale.y * 2),
            1 - cursorSizeY - cellScale.y * 2
          );
        }
        const autoUniforms = this.autoMouse.material.uniforms;
        autoUniforms.force.value.set(autoForceX, autoForceY);
        autoUniforms.center.value.set(autoCenterX, autoCenterY);
        autoUniforms.scale.value.set(cursorSz, cursorSz);
    ...
    class Viscous extends ShaderPass {
      ...
      update({ viscous: viscousVal, iterations, dt: dtVal } = {}) {
        if (!this.uniforms) return;
        let fbo_in, fbo_out;
    ...
    class Pressure extends ShaderPass {
      ...
      update({ vel, pressure: pressureVal } = {}) {
    ...
    class WebGLManager {
      ...
      render() {
        ...
        Mouse.update();
        Common.update();
        if (this.output) this.output.update(this.autoDriver);
      }
      loop() {
  ...
  // Live-update simulation options without remounting WebGL context
  useEffect(() => {
